...
def judge_prompt(chunk, start, models, first=True, seen_details=(), idxs=None):
    ...
    head = [
        ...
        "· 유형 : 개발 / 사무 / 현장 / 협업 중 하나",
        "· 세부업무 : 15자 이내 짧은 명사구 — 같은 일은 같은 이름으로 (예: '광학 설계', 'BOM 발주',",
...
def merge_details(rows, amap=None):
    ...
    LM22: ① 세부업무 병합 캐시(core/details, 사용자·Copilot 이 누적한 대표 이름)를 먼저
    적용해 signals/mm_rows/pivots 가 원천에서 대표 이름을 갖게 한다. ② 괄호 꼬리가 다른
    이름('레이아웃 검토(양산)' / '(선행)' / 꼬리 없음)은 토큰이 겹쳐도 절대 병합하지 않는다 —
    예전 토큰 규칙은 괄호를 구분자로 갈라 셋을 하나로 오병합했다(실측)."""
    if amap is None:
        amap = load_detail_alias_map()
    n = 0
    if amap:
        for r in rows:
