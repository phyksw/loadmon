# -*- coding: utf-8 -*-
r"""
details.py — 세부업무(Level 3) 표기 병합 맵 (LoadMonitor24, 보완2 이식)
...
# 상위(Level 1) 고정 범주 — 제보자의 조직 어휘 그대로다:
#   개발  = 선행·신제품·요소기술 개발 (과제는 보통 프로젝트 코드네임으로 불린다)
...
LEVEL1_ALIAS = {
    ...
    "일반업무": "공통", "표준 특허": "공통", "표준특허": "공통", "AX·자동화": "AX",
    # 현장에서 쓰는 말
...
def _level1_extra():
    r"""config.level1Set 에 적은 추가 범주 — 제보의 "등등" 자리. 기본 네 가지에 덧붙인다."""
    try:
        with open(os.path.join(ROOT, "config", "config.json"), encoding="utf-8-sig") as f:
            v = json.load(f).get("level1Set")
