...
try { $Host.UI.RawUI.WindowTitle = 'LoadMonitor22 - PC 이동 준비' } catch {}
...
function Friendly([string]$name, [string]$cmdline) {
    ...
    if ($n -eq 'cmd') { return 'LoadMonitor22 실행 창' }
...
if (-not $CheckOnly) {
    # 우리가 띄운 창(대시보드·LoadMonitor22 실행 창)이 곧 사라진다. 예고 없이 사라지면 그것 자체가
    # '오류로 꺼졌다' 로 읽힌다(감사 확정) — 죽이기 전에 먼저 알린다.
    Say '  LoadMonitor22 창(대시보드·팀 서버·샘플러)을 닫습니다 - 창이 사라지는 것은 정상입니다.' 'DarkGray'
    ...
    if ($killed.Count) {
        ...
        Write-Host '  확인했습니다: 이 폴더를 쓰고 있는 LoadMonitor22 프로그램은 이미 모두 닫혀 있었습니다'
    }
...
try {
    ...
    foreach ($w in $sh.Windows()) {
        try {
            ...
            # 구분자까지 봐야 형제 폴더(LoadMonitor22_old)를 오탐하지 않는다
...
Write-Host ''
if ($movable) {
    try { $Host.UI.RawUI.WindowTitle = 'LoadMonitor22 - PC 이동 준비 [완료]' } catch {}
    Say '  ============================================================' 'Green'
