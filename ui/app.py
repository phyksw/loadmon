# -*- coding: utf-8 -*-
"""
app.py — LoadMonitor22 로컬 HTML UI (표준 라이브러리만).

  python ui\\app.py          또는  LoadMonitor22-UI.bat 더블클릭
...
<title>LoadMonitor22 — 팀 취합</title><style>
...
<h1>팀 취합<small id="ver">LoadMonitor22</small></h1>
...
<title>LoadMonitor22</title><style>
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:'Malgun Gothic',system-ui,sans-serif;background:#f2f4f7;color:#12151a;padding:22px}
...
<h1>LoadMonitor22<small id="ver">로컬 전용 · 외부 전송 없음</small></h1>
...
<div class="card"><h2>팀 취합 업로드 <span class="state">자동 전송하지 않습니다 — 서버에 닿는 망에서 버튼으로 보냅니다</span></h2>
 ...
 (LoadMonitor22-팀취합.bat 과 같은 화면).</div>
...
$("prepmove").onclick=async()=>{
 ...
 if(!r.ok){$("prepmove").disabled=false;$("state").textContent="대기 중";
  alert("정리 창을 띄우지 못했습니다 — LoadMonitor22-이동준비.bat 을 직접 실행하세요."+(r.error?"\\n"+r.error:""));return;}
 ...
 document.body.innerHTML='<div class="wrap"><h1>PC 이동 준비</h1>'
  ...
  +'옮긴 PC 에서는 LoadMonitor22-UI.bat 을 실행하면 됩니다 — 지난 PC 데이터는 자동으로 합산됩니다.</div></div></div>';
};
$("cdiag").onclick=async()=>{
            ...
            if p and os.path.exists(p):
                ...
                if self.path.startswith("/team/full"):
                    ...
                    msg = ("<meta charset='utf-8'><body style=\"font-family:'Malgun Gothic'\">"
                           ...
                           "<p>대시보드의 [팀 취합] 버튼을 누르거나 LoadMonitor22-팀취합.bat 을 실행하세요.<br>"
                           "config.teamShareDir 설정과 팀원들의 내보내기가 선행돼야 합니다.</p></body>")
...
def main():
    ...
    print(f"[ui] LoadMonitor22 {VERSION} — {url}  (Ctrl+C 종료)")
