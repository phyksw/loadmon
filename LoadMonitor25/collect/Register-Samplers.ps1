...
param(
    ...
    [switch]$NoStart,
    [switch]$DryRun,
    [string]$TaskPrefix = 'LoadMonitor25'
)
...
$jobs = @(@{ name = "$TaskPrefix-Sampler"; script = (Join-Path $here 'Start-ActivitySampler.ps1'); desc = 'LoadMonitor25 창 샘플러 - 로그온 시 자동 시작, 실행 시간 제한 없음 (1분마다 활성 창·무입력 시간을 로컬 CSV 에 기록)' })
if ($Teams) {
    $jobs += @{ name = "$TaskPrefix-TeamsSampler"; script = (Join-Path $here 'Start-TeamsSampler.ps1'); desc = 'LoadMonitor25 팀즈 상시 샘플러 - 로그온 시 자동 시작, 실행 시간 제한 없음 (열린 팀즈 대화를 5분마다 읽어 로컬 CSV 에 누적)' }
}
...
Write-Host '[register] 완료 - 상태 확인: LoadMonitor25-수집진단.bat ([창 샘플러] 절)'
exit 0
