...
$tiS = $null
foreach ($tn in @('LoadMonitor25-Sampler', 'LoadMonitor25-TeamsSampler')) {
    $ti = Task-Info $tn
    if ($tn -eq 'LoadMonitor25-Sampler') { $tiS = $ti }
    if ($ti.found) {
...
try {
    ...
    foreach ($t in @($svcD.GetFolder('\').GetTasks(1))) {
        $tn2 = [string]$t.Name
        if ($tn2 -match "^LoadMonitor\d+-(Teams)?Sampler$" -and $tn2 -notlike 'LoadMonitor25-*') {
...
if ($actF.Count -or $tiS.found -or $sp.Count) {
    if (-not $tiS.found) { $findings.Add('창 샘플러 작업(LoadMonitor25-Sampler) 미등록 → 로그온 때마다 수동 시작해야 합니다. 등록: powershell -ExecutionPolicy Bypass -File collect\Register-Samplers.ps1 (관리자 불필요, 실행 시간 제한 없음)') }
    ...
    if ($age -gt 10 -and -not $sp.Count) { $findings.Add("창 샘플러 멈춤(마지막 샘플 ${age}분 전, 프로세스 없음) → 재시작: powershell -WindowStyle Hidden -ExecutionPolicy Bypass -File collect\Start-ActivitySampler.ps1 (등록돼 있으면 schtasks /Run /TN LoadMonitor25-Sampler)") }
