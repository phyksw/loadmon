# -*- coding: utf-8 -*-
"""
app.py — LoadMonitor25 로컬 HTML UI (표준 라이브러리만).

  python ui\\app.py          또는  LoadMonitor25-UI.bat 더블클릭
...
SAMPLER_TASK = "LoadMonitor25-Sampler"      # docs\설정가이드 §4 · collect\Register-Samplers.ps1 의 작업 이름
    ...
    try:
        ...
        if p.poll() is not None:
            how, note = "error", (f"기동 직후 종료(rc={p.returncode}) — 실행 정책·보안 정책이 막았을 수 있습니다. "
                                  "LoadMonitor25-샘플러등록.bat 으로 등록해 보세요")
            return
        how = "direct"
...
<title>LoadMonitor25 — 팀 취합</title><style>
...
<h1>팀 취합<small id="ver">LoadMonitor25</small></h1>
...
<title>LoadMonitor25</title><style>
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:'Malgun Gothic',system-ui,sans-serif;background:#f2f4f7;color:#12151a;padding:22px}
...
<h1>LoadMonitor25<small id="ver">로컬 전용 · 외부 전송 없음</small></h1>
...
<div class="card"><h2>팀 취합 업로드 <span class="state">자동 전송하지 않습니다 — 서버에 닿는 망에서 버튼으로 보냅니다</span></h2>
 ...
 (LoadMonitor25-팀취합.bat 과 같은 화면).</div>
            ...
            if p and os.path.exists(p):
                ...
                if self.path.startswith("/team/full"):
                    ...
                    msg = ("<meta charset='utf-8'><body style=\"font-family:'Malgun Gothic'\">"
                           ...
                           "<p>대시보드의 [팀 취합] 버튼을 누르거나 LoadMonitor25-팀취합.bat 을 실행하세요.<br>"
                           "config.teamShareDir 설정과 팀원들의 내보내기가 선행돼야 합니다.</p></body>")
...
def main():
    ...
    print(f"[ui] LoadMonitor25 {VERSION} — {url}  (Ctrl+C 종료)")
