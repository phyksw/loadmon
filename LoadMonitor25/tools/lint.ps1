...
# exit 0 = clean, exit 1 = findings. Used by humans and project quality hooks.
...
Push-Location $root
$devPython = Get-Command python -ErrorAction SilentlyContinue
if ($devPython) {
    python -m ruff check --no-cache core collect tools ui *.py --output-format=concise
    if ($LASTEXITCODE -ne 0) { $fail = 1 }
} else {
    $fail = 1
    Write-Output 'Python with Ruff is required for lint.'
}
Pop-Location
...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
    $fail = 1
    Write-Output 'Node.js is required for the JavaScript syntax gate.'
} elseif (Test-Path $jsChk) {
    $py = 'python'
    ...
    $fail = 1
    Write-Output 'tools\check_page_js.py is required for the JavaScript syntax gate.'
}

# --- gate 7: 배포 트리 안의 개발 PC 실경로 ---
# 실측: 주석에 개발 PC 의 실제 폴더(D:\배포 · D:\작업\Release\fw)가 남은 채 배포됐다. 자리표시자
# (<사용자>·%USERNAME%·홍길동·D:\src·D:\작업·D:\LoadMonitor25 …)는 두고, 실제 계정 홈(C:\Users\<ASCII 계정>)·
